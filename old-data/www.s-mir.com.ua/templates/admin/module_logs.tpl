...
{if $options.useorder}
{tabcontrol items="Записи" comm="Комментарии" import="Импорт/Экспорт" opt="Настройки"}
{else}
{tabcontrol items="Записи" comm="Комментарии" import="Импорт/Экспорт" opt="Настройки"}
{/if}
{else}
{if $options.useorder}
{tabcontrol items="Записи" import="Импорт/Экспорт" opt="Настройки"}
{else}
{tabcontrol items="Записи" import="Импорт/Экспорт" opt="Настройки"}
{/if}
{/if}
...
<th width="100" align="center">№ заказа</th>
<th width="100" align="center">Способ оплаты</th>
<th width="100" align="center">Статус оплаты</th>
<th width="110" align="left">Сумма</th>
...
<td><a href="javascript:getedititemform({$items[i].id})" title="Редактировать">{$items[i].id} ({$items[i].name})</a>
...
<td class="gray">{if $items[i].user.login}{$items[i].user.name} ({$items[i].user.login}){else}не задан{/if}</td>
<td width="100" align="center">{$items[i].order}</td>
<td width="100" align="center">{$items[i].payment_name}</td>
<td width="100" align="center">{if $items[i].status==2}Не оплачен{elseif $items[i].status==1}Оплачен{else}не задан{/if}</td>
<td align="left" width="110" nowrap><nobr>{$items[i].price|round:2}<span class="gray">&nbsp;{$options.valute}</span></nobr></td>
